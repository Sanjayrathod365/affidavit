...
const FabricTemplateEditor = React.forwardRef(({
  ...
  // Update handleAddText function to create semi-transparent text with grey background
  const handleAddText = () => {
    ...
    try {
      ...
      const text = new fabricLib.current.Textbox('New Text', {
        ...
        fontFamily: 'Arial',
        fontWeight: 'normal',
        backgroundColor: 'rgba(240, 240, 240, 0.7)', // Semi-transparent light grey background
      ...
      setTimeout(() => {
        try {
          ...
          addDebugLog(`[Add Text] Error in delayed check: ${err}`);
        }
      }, 300);
      ...
      addDebugLog(`[Add Text] Error adding text: ${err}`);
      notify(`Failed to add text: ${err}`, 'error');
    }
  };
  ...
  const handleAddRectangle = () => {
    ...
    try {
      ...
      notify(`Failed to add rectangle: ${err}`, 'error');
    }
  };
          ...
          <div className="col-span-3">
            <Tabs defaultValue="elements" value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="grid grid-cols-3 bg-gray-100 rounded-t-lg border border-gray-200 border-b-0 overflow-hidden">
                <TabsTrigger 
                  value="elements" 
                  className={`px-4 py-2.5 text-sm ${activeTab === 'elements' ? 'bg-white border-b-2 border-blue-500 font-medium' : 'text-gray-500'}`}
                >
                  Elements
                </TabsTrigger>
                <TabsTrigger 
                  value="placeholders" 
                  className={`px-4 py-2.5 text-sm ${activeTab === 'placeholders' ? 'bg-white border-b-2 border-blue-500 font-medium' : 'text-gray-500'}`}
                >
                  Placeholders
                </TabsTrigger>
                <TabsTrigger 
                  value="properties"
                  className={`px-4 py-2.5 text-sm ${activeTab === 'properties' ? 'bg-white border-b-2 border-blue-500 font-medium' : 'text-gray-500'}`}
                >
                  Properties
                </TabsTrigger>
              </TabsList>
                      ...
                      onClick={() => {
                        ...
                        if (fabricCanvas.current && fabricLib.current) {
                          try {
                            ...
                            addDebugLog(`[Add Circle] Error adding circle: ${err.message}`);
                            notify("Failed to add circle: " + err.message, 'error');
                          }
                        } else {
                      ...
                      onClick={() => {
                        if (fabricCanvas.current && fabricLib.current) {
                          try {
                            ...
                            notify("Failed to add line: " + err.message, 'error');
                          }
                        } else {
                          notify("Canvas is not ready");
                        }
